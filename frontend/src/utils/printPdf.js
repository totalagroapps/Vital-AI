 ...
 * Utiliza un iframe aislado para evitar la pérdida de contexto de la app o pantallas blancas
 * causadas por window.open() y window.close() en WebViews móviles.
 */

export const printHtmlContent = (title, htmlBody) => {
  // Crear un iframe invisible en el documento actual
  ...
  // Enfocar el iframe y disparar la ventana nativa de impresión
  iframe.contentWindow.focus();
  setTimeout(() => {
    try {
      iframe.contentWindow.print();
    } catch (err) {
      console.warn("Fallo al imprimir desde iframe, intentando ventana estándar:", err);
      window.print();
    }
    // Eliminar el iframe una vez presentado el diálogo
    setTimeout(() => {
      try {
