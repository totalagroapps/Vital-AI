...
const EspecialistaDetail = ({ apiUrl, doctorId, doctor: doctorProp, distanceKm, onBack, isOwnProfile, onEditProfile, onBook, onNavigate, userProfile, username, onLogout }) => {
  ...
  if (error || !doctor) {
    return (
      <div className="min-h-screen bg-base px-5 pt-4">
        <button onClick={onBack} className="mb-4 flex items-center gap-1 text-sm font-bold text-slate-500">
  ...
  const specialtyNames = (doctor.specialties || []).map((s) => translateSpecialtyName(s.name, language, t));
        ...
        <div className="flex items-center justify-between mb-5 gap-2">
          ...
          {isOwnProfile && (
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-brand-blue text-white text-sm font-bold shadow-sm hover:bg-brand-hover transition-colors shrink-0"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
          ...
          <div className="lg:col-span-3 space-y-4">
            ...
            <div className={`grid gap-2 ${hasSafeWebsite ? 'grid-cols-5' : 'grid-cols-4'}`}>
              <button onClick={wip} className="flex flex-col items-center gap-1.5 py-3 rounded-2xl border border-slate-200 bg-white text-slate-600 hover:border-brand-blue/40 transition-colors">
              ...
              <button
                ...
                className="flex flex-col items-center gap-1.5 py-3 rounded-2xl border border-slate-200 bg-white text-slate-600 hover:border-brand-blue/40 transition-colors disabled:opacity-40 disabled:hover:border-slate-200"
              >
          ...
          {hasLocation ? (
            ...
            <div className="lg:col-span-2 bg-slate-50 border border-dashed border-slate-200 rounded-2xl h-48 lg:h-64 flex flex-col items-center justify-center text-slate-400 text-center px-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mt-5">
          ...
          <InfoCard icon={<Clock size={15} className="text-brand-blue" />} title={t('detail_hours_title')}>
            {Object.keys(scheduleByDay).length === 0 ? (
              <p className="text-xs text-slate-400">{t('detail_hours_pending')}</p>
            ) : (
          ...
          <InfoCard icon={<Star size={15} className="text-amber-400" />} title={t('detail_reviews_title')}>
            <p className="text-xs text-slate-400">{t('detail_reviews_pending')}</p>
          </InfoCard>

          <InfoCard icon={<Stethoscope size={20} className="text-brand" />} title={`${t('detail_about_prefix')} ${doctor.full_name}`}>
            <div className="space-y-3 text-[15.6px]">
              <div className="flex items-start gap-2 text-slate-700">
                <Stethoscope size={18} className="text-brand shrink-0 mt-0.5" />
              ...
              {doctor.education && (
                <div className="flex items-start gap-2 text-slate-700">
                  <GraduationCap size={18} className="text-brand shrink-0 mt-0.5" />
              ...
              {insuranceNames.length > 0 && (
                <div className="flex items-start gap-2 text-slate-700">
                  <ShieldPlus size={18} className="text-brand shrink-0 mt-0.5" />
              ...
              {!doctor.clinic_name && !doctor.address && !doctor.phone && !doctor.email && !doctor.website && (
                <p className="text-slate-400 pt-1">{t('detail_no_contact')}</p>
              )}
            </div>
          </InfoCard>
          ...
          <div className="max-w-screen-2xl mx-auto">
            {offersVideo ? (
              <button
                ...
                className="w-full lg:max-w-sm lg:mx-auto flex items-center justify-center gap-2 py-3.5 rounded-2xl font-bold text-white bg-brand-blue hover:bg-brand-hover transition-colors"
              >
                <Calendar size={20} className="shrink-0" /> <span className="leading-none">{t('detail_appointment_cta')}</span>
              </button>
            ) : (
              <p className="text-center text-xs text-slate-500 py-2">{t('detail_no_video_notice')}</p>
            )}
          </div>
        </div>
      )}
