...
export default function DoctorCalendarView({ 
    ...
    if (cat.includes('rojo') || cat.includes('emergencia')) {
      return (
        <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-rose-50 text-rose-700 border border-rose-200">
          ...
          {t('doctorcalendarview_rojo_urgente')}
        </span>
      );
    }
  ...
  const renderStatusButton = (appt) => {
    ...
    const statusConfig = {
      'confirmada': { label: t('appts_status_confirmed'), bg: 'bg-blue-50 text-blue-700 border-blue-200' },
      'en_espera': { label: t('doctorcalendarview_en_sala_de_espera'), bg: 'bg-amber-100 text-amber-800 border-amber-300 animate-pulse' },
      'completada': { label: t('appts_status_completed'), bg: 'bg-emerald-100 text-emerald-800 border-emerald-300' },
      'cancelada': { label: t('appts_status_cancelled'), bg: 'bg-gray-100 text-gray-600 border-gray-200' }
    };
    ...
    return (
      <button
        ...
        title={t('doctorcalendarview_toca_para_cambiar_estado')}
        ...
        <div className="max-w-4xl mx-auto flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-black text-slate-900 tracking-tight flex items-center gap-1.5">
                ...
                <span>{t('doctorcalendarview_mi_agenda_medica')}</span>
              </h1>
              <p className="text-[11px] font-semibold text-slate-500">
                {doctorProfile?.full_name || t('default_doctor_name')} {t('doctorcalendarview_mivor_ai')}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              title={t('doctorcalendarview_actualizar_agenda')}
            >
            ...
            >
              <Plus size={15} />
              <span className="hidden sm:inline">{t('doctorcalendarview_nueva_cita')}</span>
            </button>
          </div>
      ...
      <div className="max-w-4xl mx-auto px-4 pt-4 space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
          <div className="bg-white rounded-2xl p-3 border border-gray-100 shadow-xs flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-[10px] font-semibold text-slate-500 mt-1">{t('doctorcalendarview_citas_hoy')}</div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-3 border border-gray-100 shadow-xs flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-[10px] font-semibold text-slate-500 mt-1">{t('doctorcalendarview_en_sala')}</div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-3 border border-gray-100 shadow-xs flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-[10px] font-semibold text-slate-500 mt-1">{t('doctorcalendarview_teleconsulta')}</div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-3 border border-gray-100 shadow-xs flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-[10px] font-semibold text-slate-500 mt-1">{t('doctorcalendarview_atendidos')}</div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-3 border border-gray-100 shadow-xs space-y-3">
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1 no-scrollbar">
            ...
            >
              {t('doctorcalendarview_hoy')}{appointments.filter(a => isDateToday(a.appointment_date)).length})
            </button>
            <button
            ...
            >
              {t('doctorcalendarview_manana')}{appointments.filter(a => isDateTomorrow(a.appointment_date)).length})
            </button>
            <button
            ...
            >
              {t('doctorcalendarview_todas')}{appointments.length})
            </button>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 pt-1 border-t border-gray-100">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder={t('doctorcalendarview_buscar_por_paciente_o_motivo')}
            ...
            <div className="flex items-center gap-1.5">
              ...
              >
                <option value="all">{t('doctorcalendarview_modalidad_todas')}</option>
                <option value="presencial">{t('book_modality_in_person')}</option>
                <option value="teleconsulta">{t('doctorcalendarview_teleconsulta')}</option>
              </select>
              ...
              <select
                ...
                className="px-2.5 py-1.5 bg-gray-50 border border-gray-200 rounded-xl text-xs font-semibold text-gray-700 focus:outline-none"
              >
                <option value="all">{t('doctorcalendarview_estado_todos')}</option>
                <option value="confirmada">{t('appts_status_confirmed')}</option>
                <option value="en_espera">{t('doctorcalendarview_en_sala_de_espera')}</option>
                <option value="completada">{t('appts_status_completed')}</option>
              </select>
            </div>
          </div>
        ...
        <div className="space-y-3">
          {isLoading ? (
            <div className="p-12 text-center text-gray-400">
              ...
              <p className="text-xs font-medium">{t('doctorcalendarview_cargando_agenda_de_citas')}</p>
            </div>
            ...
            <div className="bg-white rounded-3xl p-10 text-center border border-gray-100 shadow-soft">
              ...
              <h3 className="text-sm font-bold text-slate-800 mb-1">
                {appointments.length === 0 ? t('doctorcalendarview_empty_agenda_title') : t('doctorcalendarview_no_hay_citas_en_este')}
              </h3>
              <p className="text-xs text-slate-500 max-w-xs mx-auto mb-4">
                {appointments.length === 0 
                  ? t('doctorcalendarview_empty_agenda_desc') 
                  : t('doctorcalendarview_puedes_cambiar_el_filtro_de')}
              </p>
              <div className="flex flex-wrap items-center justify-center gap-2">
                ...
                >
                  <Plus size={15} />
                  <span>{t('doctorcalendarview_agendar_cita_ahora')}</span>
                </button>
              </div>
            </div>
          ) : (
                ...
                >
                  ...
                  <div className="flex items-start gap-3.5">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-slate-600 line-clamp-1">
                        <span className="font-semibold text-slate-700">{t('doctorcalendarview_motivo')}</span> {appt.reason}
                      </p>

                      <div className="flex items-center gap-3 text-[11px] text-slate-400 font-medium">
                        {appt.patient_age && <span>{appt.patient_age} {t('years')}</span>}
                        ...
                        <span>{t('doctorcalendarview_fecha')} {appt.appointment_date}</span>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-2 pt-2 sm:pt-0 border-t sm:border-t-0 border-gray-100">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        const msg = encodeURIComponent(t('doctorcalendarview_estimado_a_le_recordamos_su', { patient_name: appt.patient_name, appointment_date: appt.appointment_date, appointment_time: appt.appointment_time, appointment_type: appt.appointment_type }));
                      ...
                      title={t('doctorcalendarview_enviar_recordatorio_por_whatsapp')}
                    >
                    ...
                    >
                      <Eye size={13} />
                      <span>{t('doctorcalendarview_expediente')}</span>
                    </button>
          ...
          <div className="bg-white rounded-3xl p-6 w-full max-w-md shadow-2xl border border-gray-100 animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="flex items-center justify-between mb-4 border-b border-gray-100 pb-3">
              <h3 className="text-base font-black text-slate-900 flex items-center gap-2">
                ...
                <span>{t('doctorcalendarview_agendar_nueva_cita')}</span>
              </h3>
              <button
            ...
            <form onSubmit={handleCreateAppointment} className="space-y-3.5">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">{t('patient_name')}</label>
                <input
                  type="text"
                  required
                  placeholder={t('doctorcalendarview_ej_carmen_delgado_morales')}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">{t('col_date')}</label>
                  <input
                    type="date"
                    required
                    value={newDate}
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">{t('doctorcalendarview_hora')}</label>
                  <input
                    type="time"
                    required
                    value={newTime}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">{t('doctorcalendarview_modalidad')}</label>
                  <select
                    value={newType}
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">{t('doctorcalendarview_nivel_de_atencion_sugerido')}</label>
                  <select
                    value={newTriage}
                    onChange={(e) => setNewTriage(e.target.value)}
                    className="w-full px-3 py-2 text-xs bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-teal-500/30 outline-none"
                  >
                    <option value="Verde">{t('doctorcalendarview_verde_normal')}</option>
                    <option value="Amarillo">{t('doctorcalendarview_amarillo_urgente')}</option>
                    <option value="Rojo">{t('doctorcalendarview_rojo_emergencia')}</option>
                  </select>
                </div>
              </div>

              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">{t('doctorcalendarview_motivo_clinico')}</label>
                <textarea
                  required
                  rows={2}
                  placeholder={t('doctorcalendarview_ej_revision_post_analitica_de')}
                  value={newReason}
              ...
              <div className="pt-2 flex items-center justify-end gap-2">
                ...
                >
                  {isSubmitting ? t('saving_changes') : t('doctordirectorymod_confirmar_cita')}
                </button>
              </div>
